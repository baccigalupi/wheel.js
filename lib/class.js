/*
 * Adds methods 'subclass', 'mixin', 'mashin' to Function
 * Creates a base class 'Class' for subclassing
 */
(function(){
  var superExtend = function(base, props) {
    for(prop in props) {
      // don't copy intrinsic properties
      if ( prop.match(/prototype|arguments|__proto__/) ) { return; }

      if ( base[prop] && (typeof props[prop] == 'function' ) &&
           /\b_super\b/.test(props[prop]) ) {
        base[prop] = (function(name, func, _super){
          return function() {
            // store this in case we are calling through multiple methods
            // that use super, that way each is restored after super is used
            var exSuper = this._super;
            this._super = _super;
            var returned = func.apply(this, arguments);
            // and the restoration here!
            this._super = exSuper;
            return returned;
          };
        })(prop, props[prop], base[prop]);
      } else {
        base[prop] = props[prop];
      }
    }
    return base;
  };

  Function.mashin = function(props) {
    var prop;
    superExtend(this, props)
    return this;
  };

  Function.mixin = function(props) {
    var prop;
    superExtend(this.prototype, props);
    return this;
  };

  var initializing = false;
  Function.subclass = function(iProps, cProps) {
    var proto;

    initializing = true;
    proto = new this();
    initializing = false;

    // high level constructor
    function Subclass() {
      if (!initializing && this.initialize) {
        this.initialize.apply(this, arguments);
      }
    }

    // add existing class method from old class
    for( prop in this ) {
      var prop;
      if ( prop == 'prototype' ) { return; }
      Subclass[prop] = this[prop];
    }

    Subclass.mashin(cProps);
    Subclass.prototype = superExtend(proto, iProps);
    Subclass.superclass = this;
    Subclass.prototype.constructor = Subclass;
    Subclass.prototype._class = Subclass.prototype.constructor; // more intuitive access
    return Subclass
  };

  // export the Class variable
  this.Class = Function.subclass();
})();

