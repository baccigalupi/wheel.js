Wheel.BodyListener = Wheel.View.subclass({
  ...
  listenForTouchStart: function() {
    ...
    this.$.bind('touchstart', function(e) {
      ...
      console.log("is there a timeout? " + self.touchTimeout);
  ...
  listenForTouchMove: function() {
    ...
    this.$.bind('touchmove', function(e) {
      ...
      if (Math.abs(self.touch.x1 - self.touch.x2) >= this._class.SWIPE_TOLERANCE ||
          Math.abs(self.touch.y1 - self.touch.y2) > this._class.SWIPE_TOLERANCE) {
  ...
  listenForTouchEnd: function () {
    ...
    this.$.bind('touchend', function(e) {
      ...
      if (self.touch.isDoubleTap) {
        ...
        self.touch = {};
      } else if (self.touch.isSwipe) {
        console.log('is touchhold');
        self.touch = {};
      } else if (self.touch.last) {
        console.log('waiting to trigger tap, if after timeout ... no double tap');
  ...
  _testForTouchHold: function(e) {
    ...
    if ((this.touch.last && (Date.now() - this.touch.last >= this._class.HOLD_DELAY)) &&
        (Math.abs(x1 - x2) <= this._class.HOLD_TOLERANCE) &&
        (Math.abs(y1 - y2) <= this._class.HOLD_TOLERANCE)) {
